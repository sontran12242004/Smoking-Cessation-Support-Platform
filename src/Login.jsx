...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Chuyển hướng theo vai trò (role) và membership status
      switch (user.role) {
        case 'ADMIN':
          navigate('/admin/dashboard');
          break;
        case 'MEMBERS':
          // Kiểm tra membership status để redirect đúng trang
          if (user.membershipInfo && user.membershipInfo.hasActiveMembership && !user.membershipInfo.expired) {
            // Có membership active và chưa hết hạn -> Elite Member
            navigate('/elite/home');
          } else {
            // Không có membership hoặc đã hết hạn -> Standard Member
