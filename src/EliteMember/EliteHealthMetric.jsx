...
const EliteHealthMetric = () => {
  ...
  if (!userId) {
    try {
      ...
      console.error('EliteHealthMetric - error parsing localStorage user:', error);
    }
  }
  ...
  useEffect(() => {
    ...
    // Use ApiService for available methods
    Promise.all([
      ApiService.getDaysSmokeFree(userId).catch(() => '--'),
      ApiService.getMoneySaved(userId).catch(() => '--'),
      ApiService.getHealthImproved(userId).catch(() => '--'),
      ApiService.getLungCancerRisk(userId).catch(() => '--'),
      ApiService.getHeartDiseaseRisk(userId).catch(() => '--')
    ]).then(([daysData, moneyData, healthData, lungData, heartData]) => {
      // Display exact values from API without any rounding
      setDaysSmokeFree(daysData);
      setMoneySaved(moneyData);
      setHealthImproved(healthData);
      setLungCancerRisk(lungData);
      setHeartDiseaseRisk(heartData);
    }).catch(error => {
      console.error('Error fetching main health metrics:', error);
    });

    // For other endpoints not in ApiService, use fetch with auth headers
