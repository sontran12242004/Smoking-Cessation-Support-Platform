...
function EliteDashboard() {
  ...
  const { getUserName, logout, user, loading: userLoading } = useUser();
  ...
  if (!userId) {
    try {
      ...
      if (storedUser) {
        ...
        console.log('EliteDashboard - fallback userId from localStorage:', userId);
      }
    } catch (error) {
      console.error('EliteDashboard - error parsing localStorage user:', error);
    }
  }

  console.log('EliteDashboard - userId:', userId, 'userLoading:', userLoading);
  ...
  // Helper function to display raw values from API without rounding
  const formatNumber = (value, decimals = 0) => {
    if (value === null || value === undefined || value === '--') return '--';
    const num = typeof value === 'string' ? parseFloat(value) : value;
    if (isNaN(num)) return '--';
    // Handle 0 correctly - should display as 0, not --
    if (num === 0) return '0';
    // Return the exact value from API without rounding
    return decimals > 0 ? num.toFixed(decimals) : num.toString();
  };

  // Helper function to ensure chart data is valid array
  const formatChartData = (data) => {
    if (Array.isArray(data) && data.length > 0) {
      return data;
    }
    // Return default chart data if API doesn't return valid array
    return [
      { month: 'Jan', value: 0 },
      { month: 'Feb', value: 0 },
      { month: 'Mar', value: 0 },
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Helper function to safely extract numeric values from API responses
        const extractValue = (response) => {
          if (typeof response === 'number') return response;
          if (typeof response === 'string') {
            const parsed = parseFloat(response);
            return isNaN(parsed) ? response : parsed;
          }
          if (response && typeof response === 'object') {
            // Try common property names
            const value = response.value || response.data || response.result || response.count || response;
        ...
        const extractPromiseValue = (result) => {
          if (result.status === 'fulfilled') {
            return result.value;
          } else {
            console.error('API call failed:', result.reason);
            return '--'; // Show -- for failed API calls instead of mock data
          }
        };

        const actualLungCancerRisk = extractPromiseValue(lungCancerRiskData);
        const actualHeartDiseaseRisk = extractPromiseValue(heartDiseaseRiskData);
        const actualDaysSmokeFree = extractPromiseValue(daysSmokeFreeData);
        const actualMoneySaved = extractPromiseValue(moneySavedData);
        const actualHealthImproved = extractPromiseValue(healthImprovedData);

        setHealthMetrics({
          daysSmokeFree: actualDaysSmokeFree,
          daysToNext: "--",
          moneySaved: actualMoneySaved,
          healthImproved: actualHealthImproved,
          lungsCapacity: "--",
          heartRate: "--",
        });
        setLungCancerRisk(formatNumber(actualLungCancerRisk));
        setHeartDiseaseRisk(formatNumber(actualHeartDiseaseRisk));
        setDaysSmokeFree(formatNumber(actualDaysSmokeFree));
        setMoneySaved(formatNumber(actualMoneySaved)); // Keep exact value from API
        setHealthImproved(formatNumber(actualHealthImproved));
        setHealthImprovementData(formatChartData(actualHealthImproved));
    ...
    <div className="dashboard-bg" style={{ background: `url(${journeyPath}) center/cover no-repeat`, position: 'relative' }}>
      ...
      <div className="dashboard-main">
        ...
        <div className="dashboard-cards-row">
          {/* Days Smoke-Free from /health-metrics/days-free */}
          <div className="dashboard-card">
            ...
            <div className="dashboard-card-desc">
              {daysSmokeFree !== '--' && daysSmokeFree > 0 ? Math.max(0, 30 - (daysSmokeFree % 30)) : '--'} days until next milestone
            </div>
          </div>
          {/* Money Saved from /health-metrics/money-saved */}
          <div className="dashboard-card">
            <span className="dashboard-card-icon">💵</span>
          ...
          {/* Health Improved from /health-metrics/percent/health-improved */}
          <div className="dashboard-card">
            <span className="dashboard-card-icon">💚</span>
        ...
        <div className="dashboard-cards-row-small">
          {/* Lung Cancer Risk from /health-metrics/risk/lung-cancer */}
          <div className="dashboard-card-small">
            ...
            <div className="dashboard-card-small-content">
              <div className="dashboard-card-small-title">Lung Cancer Risk</div>
              <div className="dashboard-card-small-value">
                -{lungCancerRisk}%
              </div>
              <div className="dashboard-card-small-description">
                Your lung cancer risk has decreased since quitting smoking.
              </div>
            </div>
          </div>
          {/* Heart Disease Risk from /health-metrics/risk/heart-disease */}
          <div className="dashboard-card-small">
            <span className="dashboard-card-small-icon">💓</span>
            <div className="dashboard-card-small-content">
      ...
      <section className="analytics-section">
        ...
        <div className="analytics-cards-row">
          <div className="analytics-card">
            ...
            <div className="analytics-card-content" style={{ width: '100%', height: 220 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={Array.isArray(healthImprovementData) ? healthImprovementData : []}>
          ...
          <div className="analytics-card">
            ...
            <div className="analytics-card-content" style={{ width: '100%', height: 220 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={Array.isArray(successRateData) ? successRateData : []}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="month" />
                  <YAxis />
                  <Tooltip />
