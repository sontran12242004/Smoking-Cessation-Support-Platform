...
function DailyCheckIn() {
  ...
  // Load existing daily process for today
  useEffect(() => {
    const loadTodayProcess = async () => {
      if (!user?.userId) return;
      
      try {
        const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
        const processData = await ApiService.getDailyProcessByDate(user.userId, today);
        
        if (processData) {
          setExistingProcess(processData);
          // Pre-fill form with existing data
          setSmokedToday(processData.smoked ? 'yes' : 'no');
          setCigaretteStrength(processData.cigaretteStrength || 0);
          setFeeling(processData.mood || null);
          setCravingTrigger(processData.cravingTrigger || null);
          setConfidence(processData.confidence || null);
          setCigaretteCount(processData.cigarettesSmokedToday || '');
          setCigarettePrice(processData.priceSmoked || '');
        }
      } catch (err) {
        // No existing process for today is normal
        console.log('No existing process for today');
      }
    };
  ...
  const handleSubmitDailyProgress = async () => {
    ...
    // Validation
    if (smokedToday === null) {
      setError('Please answer if you smoked today');
      return;
    }

    if (smokedToday === 'yes' && (!cigaretteCount || !cigarettePrice)) {
      setError('Please enter cigarette count and price');
      return;
    }

    if (!feeling || !cravingTrigger) {
    ...
    try {
      ...
      const dailyProgressData = {
        processId: existingProcess?.processId || Math.floor(Math.random() * 1000000),
        memberId: user.userId,
        date: new Date().toISOString(),
        cigaretteStrength: parseInt(cigaretteStrength) || 0,
        priceSmoked: smokedToday === 'yes' ? parseFloat(cigarettePrice) || 0 : 0,
        smoked: smokedToday === 'yes',
        mood: feeling,
        cravingTrigger: cravingTrigger,
        confidence: confidence || feeling, // Use feeling as confidence if not set
        cigarettesSmokedToday: smokedToday === 'yes' ? parseInt(cigaretteCount) || 0 : 0
      };

      await ApiService.submitDailyProgress(user.userId, dailyProgressData);
      ...
      console.error('Error submitting daily progress:', err);
      setError(err.response?.data?.message || 'Failed to submit daily progress');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container">
      ...
      <main className="main-content">
        <div style={{ position: 'relative', flexGrow: 1, width: '100%', background: `url(${journeyPath}) center/cover no-repeat` }}>
          {/* Overlay */}
          <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: '#DFF5DE', opacity: 0.7, zIndex: 0 }} />
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', padding: '40px' }}>
            <div className="check-in-card-container">
              ...
              {success && (
                <div style={{
                  ...
                  ✅ Daily progress saved successfully! Redirecting...
                </div>
              )}

              <button 
                className="save-responses-button" 
              ...
              >
                {loading ? 'Saving...' : success ? 'Saved!' : existingProcess ? 'Update Progress' : 'Save Responses'}
              </button>
            </div>
          </div>
        </div>
