...
function PackagePage() {
        ...
        const transformedPackages = packagesData.map((plan) => {
          ...
          if (features.length === 0) {
            features = [
              ...
              { text: "Community access", included: plan.price > 0 },
              { text: "Weekly tips", included: true },
              { text: "Personal coach", included: plan.price > 50 },
              { text: "Advanced analytics", included: plan.price > 100 },
              { text: "Email reminders", included: plan.price > 0 },
            ];
          }
          ...
          return {
            ...
            name: plan.name,
            price: plan.price,
            priceFormatted: plan.price === 0 ? "Free" : new Intl.NumberFormat('vi-VN', {
              style: 'currency',
              currency: 'VND'
            }).format(plan.price),
            duration: plan.duration >= 365 ? 
              `${Math.floor(plan.duration / 365)} year${Math.floor(plan.duration / 365) > 1 ? 's' : ''}` : 
  ...
  const handleSelectPackage = async (pkg) => {
    try {
      setProcessingPackageId(pkg.planID);
      console.log('Buying package with ID:', pkg.planID);
      
      // Call API to buy package
      const response = await ApiService.buyMembershipPlan(pkg.planID);
      console.log('Buy package response:', response);
      
      // Handle response based on package type
      if (pkg.price === 0) {
        // Free package - no payment needed
        alert('Successfully subscribed to free package!');
        navigate('/standard/home'); // Navigate to appropriate dashboard
      } else {
        // Paid package - check if response contains VNPay URL
        let paymentUrl = null;
        
        if (typeof response === 'string') {
          if (response.includes('vnpayment.vn')) {
            paymentUrl = response;
          } else if (response.startsWith('@')) {
            // Handle @URL format from your example
            paymentUrl = response.substring(1); // Remove @ prefix
          }
        ...
        if (paymentUrl) {
          ...
          alert('Package purchase initiated. Please check your account status.');
          navigate('/elite/home');
        }
      }
    } catch (error) {
    ...
    <div>
      ...
      <main>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div className="packages-container" style={{ flexDirection: 'row', justifyContent: 'center', flexWrap: 'wrap', gap: '30px' }}>
            {/* Loading State */}
            {loading && (
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px', flexDirection: 'column' }}>
                <div style={{ fontSize: '18px', color: '#666', marginBottom: '10px' }}>Loading packages...</div>
                <div style={{ 
            ...
            {error && !loading && (
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px', flexDirection: 'column' }}>
                <div style={{ fontSize: '18px', color: '#f44336', marginBottom: '10px' }}>⚠️ {error}</div>
                <button
            ...
            {/* Packages Display */}
            {!loading && !error && packages.length > 0 && packages.map((pkg, index) => (
              <div key={pkg.planID} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              minWidth: 320,
                  maxWidth: 350,
              borderRadius: 18,
                  border: pkg.price > 0 ? '2px solid #FBC02D' : '1.5px solid #b6d7b0',
              ...
              position: 'relative',
                ...
                onMouseOver={e => { 
                  e.currentTarget.style.boxShadow = '0 8px 32px rgba(60,120,60,0.18)'; 
                  e.currentTarget.style.transform = 'translateY(-4px) scale(1.015)'; 
                }}
                onMouseOut={e => { 
                  e.currentTarget.style.boxShadow = '0 4px 24px rgba(60,120,60,0.10)'; 
                  e.currentTarget.style.transform = 'none'; 
                }}
                >
                  {/* Popular badge for paid packages */}
                  {pkg.price > 0 && (
              ...
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                    <span style={{ fontSize: 24, fontWeight: 900, color: '#222', letterSpacing: 1 }}>
                      {pkg.priceFormatted}
                    </span>
                    <span style={{ fontSize: 18, fontWeight: 400, color: '#222', marginLeft: 4 }}>
                      /{pkg.duration}
                    </span>
              </div>
              ...
              <ul className="features" style={{ color: '#222', margin: 0, padding: 0, listStyle: 'none', fontSize: 16, fontWeight: 400 }}>
                    {pkg.features.map((feature, featureIndex) => (
                      <li key={featureIndex} style={{ display: 'flex', alignItems: 'center', marginBottom: 14, gap: 10 }}>
                        <span style={{ 
                          color: feature.included ? '#1ca21c' : '#c62828', 
                          fontSize: 22, 
                          marginRight: 8 
                        }}>
                          {feature.included ? '✓' : '✖'}
                        </span>
                        <span style={{ color: feature.included ? '#222' : '#c62828' }}>
                          {feature.text}
                        </span>
                      </li>
                    ))}
            ...
            <div style={{ textAlign: 'center', marginTop: 0 }}>
                  <button 
                    ...
                    disabled={processingPackageId === pkg.planID}
                    style={{
                      background: processingPackageId === pkg.planID ? '#ccc' : '#4CAF50',
                      color: '#fff',
                      border: 'none',
                      borderRadius: '6px',
                      padding: '12px 32px',
                      fontSize: '18px',
                      fontWeight: '600',
                      cursor: processingPackageId === pkg.planID ? 'not-allowed' : 'pointer',
                      marginTop: '8px',
                      transition: 'background 0.2s, transform 0.15s, box-shadow 0.18s',
                      boxShadow: processingPackageId === pkg.planID ? 'none' : '0 2px 8px #4caf5022'
                    }}
                  >
                    {processingPackageId === pkg.planID ? 
                      (pkg.price === 0 ? 'Starting Free Package...' : 'Processing Payment...') : 
                      (pkg.price === 0 ? 'Start Free Package' : `Buy for ${pkg.priceFormatted}`)
                    }
              </button>
            </div>
                    </div>
